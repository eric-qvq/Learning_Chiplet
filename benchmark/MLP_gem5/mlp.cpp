...
void mkfile(char *fileName) {
    ...
    if (file != NULL)
        std::cout << fileName << "创建成功" << std::endl;
    fclose(file);
}
...
void stopGpuWorkers() {
    ...
    std::cout << "stop gpu workers" << std::endl;
    InterChiplet::sendMessage(0, 1, 0, 0, Mat1_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(0, 2, 0, 0, Mat1_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(0, 3, 0, 0, Mat1_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(0, 4, 0, 0, Mat1_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(1, 1, 0, 0, Mat1_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(1, 2, 0, 0, Mat1_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(1, 3, 0, 0, Mat1_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(1, 4, 0, 0, Mat1_size, 2 * sizeof(int64_t));

    InterChiplet::sendMessage(0, 1, 0, 0, Mat2_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(0, 2, 0, 0, Mat2_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(0, 3, 0, 0, Mat2_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(0, 4, 0, 0, Mat2_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(1, 1, 0, 0, Mat2_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(1, 2, 0, 0, Mat2_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(1, 3, 0, 0, Mat2_size, 2 * sizeof(int64_t));
    InterChiplet::sendMessage(1, 4, 0, 0, Mat2_size, 2 * sizeof(int64_t));

    std::cout << "Mat1_size[0]: " << Mat1_size[0] << " Mat1_size[1]: " << Mat1_size[1] << std::endl;
    std::cout << "Mat2_size[0]: " << Mat2_size[0] << " Mat2_size[1]: " << Mat2_size[1] << std::endl;

    delete[] Mat1_size;
    delete[] Mat2_size;
    std::cout << "end" << std::endl;
    // int64_t stop1_flag[2] = {0, 0};
    // for (int x = 0; x <= 1; ++x) {
    //     for (int y = 1; y <= 4; ++y) {
    //         InterChiplet::sendMessage(x, y, srcX, srcY, stop1_flag, 2 * sizeof(int64_t));
    //         InterChiplet::sendMessage(x, y, srcX, srcY, stop1_flag, 2 * sizeof(int64_t));
    //     }
    // }
}

void BPNeuralNetwork(int Input_size, std::vector<int> Hidden_size, int Output_size, int SrcX, int SrcY) {
    srcX = SrcX;
    srcY = SrcY;
...
void ToGPU(double *mat1, double *mat2, int fst_Row, int fst_Col, int sec_Row, int sec_Col,
    ...
    for (size_t i = 0; i < dev1.size(); i++) {
        std::vector<std::vector<double>> res_temp(dev1[i].size(), std::vector<double>(dev2[i][0].size()));
    ...
    for (size_t i = 0; i < dev1.size(); i++) {
        ...
        THREAD.push_back(std::thread(GpuMultiply, Dev1, Dev2, dev1[i].size(), dev1[i][0].size(), dev2[i].size(),
...
void backward(std::vector<std::vector<double>> x, std::vector<std::vector<double>> &y, double learning_rate) {
    ...
    for (int i = weight.size() - 1; i >= 0; i--) {
        ...
        std::thread t1(ToGPU, Weight, Dz, weight[i][0].size(), weight[i].size(), dz.size(), dz[0].size(),
                       std::ref(deltas_pre), 1);
...
void train(std::vector<std::vector<double>> &x, std::vector<std::vector<double>> y, int num_iterations = 1000,
...
bool readDataFromJSON(const std::string &filename, std::vector<std::vector<double>> &x_train,
                      std::vector<std::vector<double>> &x_test, std::vector<std::vector<double>> &y_train,
